...
종료 메서드 추론  
기본으로 close, shutdown라는 메서드 이름을 추론해서 호출해준다  
결론  
외부 라이브러리 콜백이 필요할 때 사용한다 ★

#### @PostConstruct, @PreDestroy 애노테이션 지원  
@PostConstruct : init ( 초기화 콜백 )  
@PreDestroy : close ( 소멸전 콜백 )  
특징  
최신 스프링에서 가장 권장하는 방법  
자바 표준이다  
컴포넌트 스캔과 잘 어울린다  
외부 라이브러리에는 적용하지 못한다 해당 경우에는 @Bean 기능을 사용한다

## 빈 스코프
### 빈 스코프란?
빈이 존재할 수 있는 범위를 뜻한다  
종류  
싱글톤 : 기본 스코프, 스프링 컨테이너의 시작 -> 종료로 가장 넓은 범위의 스코프  
프로토타입 : 프로토타입 빈의 생성과 의존관계 주입까지만 관여하는 스코프  
request : 웹 요청이 들어오고 나갈때 까지 유지되는 스코프

### 프로토타입 스코프
새로운 빈( 객체 )을 생성하고 필요한 의존관계 주입  
스프링 컨테이너가 종료될 때 @PreDestroy 같은 종료 메서드가 실행되지 않는다  

####문제점  
싱글톤 빈과 함께 사용시 의존관계 주입시에만 빈을 생성한다  
프로토타입 빈은 사용시마다 생성해주는 것에 의미가 있다

#### 해결법 Provider 
ObjectProvider : 라이브러리 필요 X, 스프링에 의존 ㅇ  
JSR-330 Provider : 기능 단순, 라이브러리 필요 ㅇ, 스프링에 의존 X 

### 웹 스코프
웹 환경에서만 동작한다  
스코프의 종료시점까지 관리한다  
종료 메서드가 호출된다  
클라이언트 요청 기준 전용 스코프 생성
...
#### 문제점
스프링 애플리케이션을 실행하는 시점에 request 스코프 빈은 생성되지 않는다  
이 빈은 실제 고객의 요청이 와야 생성할 수 있다

#### 해결방안
객체조회를 꼭 필요한 시점까지 지연처리 한다  
provider 사용  
프록시 사용

#### 프록시
가상의 빈을 생성해두고 요쳥이 오면 내부에서 request 빈을 요청한다
